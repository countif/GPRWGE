...
class GEWORKER(params: Map[String, String]) extends GraphEmbedding(params: Map[String, String]){
  ...
  override def initModel(sampler: BaseSampler, bcMeta: Broadcast[DistributionMeta]): GEModel = {
    new GEWORKERModel(sampler, bcMeta, dimension, numNodePerRow, numParts)
  }
  ...
  override def train(batchData: RDD[(Int, PairsDataset)], bcMeta: Broadcast[DistributionMeta],
    ...
    val gePSModel: GEWORKERModel = gEModel.asInstanceOf[GEWORKERModel]
    ...
    val (batchLoss, batchCnt) = batchData.mapPartitionsWithIndex((Partitionid,iterator) => {
      ...
      val dstindicesSet: IntOpenHashSet = new IntOpenHashSet()
      for(i <- 0 until(negArray.length)){
        dstindicesSet.add(negArray(i))
      }
  ...
  def psTrainOnePair(srcVec: Array[Float],srcId:Int, dstId: Int, negArray: Array[Int],
    ...
    //src offset
    val srcOffset = srcindex2offset.get(srcId) * dimension
